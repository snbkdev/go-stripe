...
func (m *DBModel) GetWidget(id int) (Widget, error) {
	...
	row := m.DB.QueryRowContext(ctx, `select id, name, description, inventory_level, price,
		coalesce(image, ''), is_recurring, plan_id, created_at, updated_at from widgets where id = $1`, id)
...
func (m *DBModel) GetUserByEmail(email string) (User, error) {
	...
	row := m.DB.QueryRowContext(ctx, `select id, first_name, last_name, email, password, created_at, updated_at from users 
		where email = $1`, email)
...
func (m *DBModel) GetAllUsers() ([]*User, error) {
	...
	query := `select id, last_name, first_name, email, created_at, updated_at from users
	...
	for rows.Next() {
		var u User
		err = rows.Scan(
			&u.ID, &u.LastName, &u.FirstName, &u.Email, &u.CreatedAt, &u.UpdatedAt,
		)
